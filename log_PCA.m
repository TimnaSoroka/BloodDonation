...
for i = 1:numel(subjData1)
 ...
 [BeforeC{i}, AfterC{i},donation{i},CBefore_LI{i}, CAfter_LI{i},NCdonation{i}]=extract_timings_needle_walk_in_chair2(i,norm, IntLength,subjData1);
...
nSubj      = size(vals_before, 2);
nRepeats   = 100;
holdoutFrac = 0.7;  % 0.4 → 40% test subjects, etc.
...
for r = 1:nRepeats
    ...
    % ---- subject-wise split ----
    cvSubj = cvpartition(nSubj, 'HoldOut', holdoutFrac);
    subjTrain = training(cvSubj);
    subjTest  = test(cvSubj);

    trainIdx_rows = [subjTrain; subjTrain];
    testIdx_rows  = [subjTest;  subjTest];

    X_train = X(trainIdx_rows,:);
    Y_train = Y(trainIdx_rows,:);
 


    n=size(X_train,1)/2;
% X_train(1:nn,:)=X1(1:nn,:);
% X_train(n+1:n+nn,:)=X1(nn+1:end,:);

    X_test  = X(testIdx_rows,:);
    Y_test  = Y(testIdx_rows,:);
